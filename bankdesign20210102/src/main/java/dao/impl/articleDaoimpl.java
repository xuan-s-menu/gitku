...
public class articleDaoimpl implements articelDao {
    ...
    public List<article> findarticlebyuserid(String userid) throws Exception {
        System.out.println("进入了通过用户id找留言层");
        String sql="select * from article where userid = ?";
        QueryRunner qr= util.BaseUtils.getQueryRunner();
        List<article> list=qr.query(sql, new BeanListHandler<article>(article.class),userid);
        return list;
    }
}
